...
**为什么不用固定的 252**：252 是美股的惯例值。A 股一年的交易日数不一样（2026 年是 242 天），套用 252 会把每日波动算小约 2%。
...
> **口径提醒**：252 是美股惯例值，A 股一年的交易日数不一样（2026 年是 242 天）；而 VIX 的期限按**日历日**（365）折算。**两个年化基准不一致，会让 VRP 系统性偏小**（实测偏低 3–7 个点）。
